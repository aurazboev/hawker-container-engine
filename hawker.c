...
child_exec (void * arg)
{
        ...
        char *base_img_path = hkr_get_img_path();
        char *full_img_path = malloc(strlen(base_img_path) + strlen(p->img) + 1); //basepath
        if (!full_img_path) {
            ERRSTR("Failed to allocate memory for full image path");
            exit(EXIT_FAILURE);
        }
        sprintf(full_img_path, "%s/%s", base_img_path, p->img);

        // (1) Change our root directory to the new directory for the image
        if (chroot(full_img_path) != 0) {
            ERRSTR("Failed to change root to %s", full_img_path);
            free(full_img_path);
            exit(EXIT_FAILURE);
        } else{
        ...
        if (chdir("/") != 0) { // After chroot, the new root is at "/"
            ERRSTR("Failed to change directory to new root %s", "/");
            exit(EXIT_FAILURE);
        } else{
            char cwd[PATH_MAX];
            if (getcwd(cwd, sizeof(cwd)) != NULL) {
                ...
                ERRSTR("Failed to get current working directory");
                exit(EXIT_FAILURE);
            }
        }
        ...
        if (execvp(p->argv[0], p->argv) == -1) {
            ERRSTR("Failed to execute command %s", p->argv[0]);
            exit(EXIT_FAILURE);
        }
...
main (int argc, char **argv)
{
        ...
        clone_flags = CLONE_NEWUSER | CLONE_NEWPID | CLONE_NEWNS | CLONE_NEWIPC | CLONE_NEWNET | CLONE_NEWUTS | SIGCHLD;
        ...
        if (child_stack == NULL) {
            ERRSTR("Failed to allocate memory for child stack");
            exit(EXIT_FAILURE);
        }
        ...
        set_cpu_share(pid,cpu_limit_user);
        set_mem_limit(pid,mem_limit_user);
